...
void Ball::update(P1 player1, P2 player2) {
	...
	pos.x += vel.x;
	pos.y += vel.y;
	if (vel.x < 0 && (pos.x - player1.getPos().x) < 16 && (pos.x - player1.getPos().x) > -16 && (pos.y - player1.getPos().y) < 32 && (pos.y - player1.getPos().y) > -16) {
		vel.x = -vel.x*1.1;
		shape.setFillColor(sf::Color::Red);
	}
	if (vel.x > 0 && (pos.x - player2.getPos().x) < 16 && (pos.x - player2.getPos().x) > -16 && (pos.y - player2.getPos().y) < 32 && (pos.y - player2.getPos().y) > -16) {
		vel.x = -vel.x*1.1;
		shape.setFillColor(sf::Color::Blue);
	}
